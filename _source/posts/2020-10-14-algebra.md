...
[Haskell][haskell]’s monads have put people in dread, but they’re nothing than
...
and `Seq` has a `filter` method more expressive than `flatMap`:
...
A monoid is a structure that supplies and addition operations and an unit
instance for a type.
...
enough to supply another type implicit monoid, for instance a `DoubleMonoid`, no
need for changing the `reduce` function.
